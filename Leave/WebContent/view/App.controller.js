							...
							.extend(
									"Leave.view.App",
									{
										...
										handleChange : function() {
											var sSelCombo = this.getView()
													.byId("approver")
													.getSelectedItem()
													.getText();
											document
													.getElementById("app--approver-inner").readOnly = false;
											document
													.getElementById("app--approver-inner").value = sSelCombo;
											document
													.getElementById("app--approver-inner").readOnly = true;

										},

										onAfterRendering : function() {
											var oPage = this.getView().byId(
													"myPage");
											var oCal = this.getView().byId(
													"Calendar");
											this.getView().byId("history");
											var oCombo = this.getView().byId(
													"approver");
											...
											// $('#approver').blur();
											// });
											oCombo.onAfterRendering = function() {
												if (oCombo.getSelectedItem() == null) {
													document
															.getElementById("app--approver-inner").readOnly = true;
												} else {
													document
															.getElementById("app--approver-inner").value = oCombo
															.getSelectedItem()
															.getText();
													document
															.getElementById("app--approver-inner").readOnly = true;
										...
										addLeave : function(evt) {
											...
											var sDateSt = this.getView().byId(
													"selectedDateFrom")
													.getText();
											var sDateEn = this.getView().byId(
													"selectedDateTo").getText();
											var sLimit = +this.getView().byId(
													"res").getText();
											var sSelectedRange = +this
													.getView().byId("select")
													.getText();
											var sApprover = this.getView()
													.byId("approver")
													.getSelectedItem();
											if ((sDateSt !== "Выберите значение на календаре")
													& (sDateEn !== "Выберите значение на календаре")
													& (sDateEn !== "Дата не выбрана")
													& (sApprover !== null)) {
												if (sLimit < sSelectedRange) {
													sap.m.MessageToast
															.show("Количество выбранных дней превышает лимит, отправление заявки невозможно");
												} else {
													sap.m.MessageToast
															.show("Заявка отправлена на утверждение");
													var oReq = {
														"ReqId" : curId,
														"DataSt" : sDateSt,
														"DataEn" : sDateEn,
										...
										_updateText : function(oCalendar) {
											var oSelectedDateFrom = this
													.getView().byId(
															"selectedDateFrom");
											var oSelectedDateTo = this
													.getView().byId(
															"selectedDateTo");
											var aSelectedDates = oCalendar
													.getSelectedDates();
											var aSelectedRange = this.getView()
													.byId("select");
											var oDateSt;
											var oDateEn;
											if (aSelectedDates.length > 0) {
												oDateSt = aSelectedDates[0]
														.getStartDate();
												if (oDateSt) {
													oSelectedDateFrom
															.setText(this.oFormatYyyymmdd
																	.format(oDateSt));
												} else {
													oSelectedDateTo
															.setText("Дата не выбрана");
												}
												oDateEn = aSelectedDates[0]
														.getEndDate();
												if (oDateEn) {
													oSelectedDateTo
															.setText(this.oFormatYyyymmdd
											...
											if ((oDateSt !== null)
												...
												// var
												// iLimit=this.getView().getModel("requests").getProperty("/Limit");
										...
										_selectWeekInterval : function(iDays) {
											...
											var oCalendar = this.getView()
													.byId("Сalendar");

											oCalendar.removeAllSelectedDates();
											oCalendar
													.addSelectedDate(new DateRange(
															{
																startDate : oMonday,
																endDate : oSunday
															}));
